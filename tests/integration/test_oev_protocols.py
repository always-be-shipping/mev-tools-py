...
class TestOEVProtocolsIntegration:
    ...
    def test_aave_v3_real_liquidation_detection(
        ...
        for test_case in test_cases[:1]:  # Test first case to avoid rate limits
            ...
            try:
                ...
                # Test detection
                is_liquidation = processor.is_liquidation_transaction(tx, receipt["logs"])
                        ...
                        try:
                            ...
                            if "market_id" in decoded:
                                ...
                                try:
                                    ...
                                    print(f"Could not get market info for {market_id}: {e}")
    ...
    def test_cross_protocol_block_analysis(
        ...
        for block_number in range(start_block, end_block):
            try:
                block = web3_instance.eth.get_block(block_number, full_transactions=True)
                    ...
                    try:
                        ...
                        for protocol_name, processor in protocol_processors.items():
                            if processor.is_liquidation_transaction(tx, receipt["logs"]):
                                ...
                                print(f"Found {protocol_name} liquidation in block {block_number}: {tx['hash'].hex()}")
        ...
        print(f"Analyzed blocks {start_block}-{end_block}: {total_liquidations} total liquidations")
            ...
            for test_case in test_cases[:1]:  # One per protocol
                ...
                try:
                    ...
                    if processor.is_liquidation_transaction(tx, receipt["logs"]):
                        ...
                        print(f"{protocol_name} liquidation {tx_hash}: {gas_used:,} gas, {cost_eth:.6f} ETH")
                    ...
                    print(f"Could not analyze gas for {protocol_name} transaction {tx_hash}: {e}")
    ...
    def test_protocol_event_structure_validation(
        ...
        for protocol_name, processor in protocol_processors.items():
            ...
            required_methods = ["decode_liquidation", "enrich_event", "is_liquidation_transaction"]
