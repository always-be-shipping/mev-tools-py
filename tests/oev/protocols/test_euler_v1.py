...
class TestEulerProtocolProcessor:
    ...
    @pytest.fixture
    def sample_logs(self, sample_liquidation_log: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def test_decode_liquidation_success(
        ...
        with patch.object(processor.liquidation_event, 'process_log', return_value=mock_decoded_log) as mock_process_log:
        ...
        assert result["underlying_asset"] == "0xa0b86a33e6ba3b93b63e1fbb4f4bb4f4bb4f4bb4"
        assert result["collateral_asset"] == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
        
        # Check debt_repaid structure
        assert result["debt_repaid"]["token"] == "0xa0b86a33e6ba3b93b63e1fbb4f4bb4f4bb4f4bb4"
        assert result["debt_repaid"]["amount"] == "100000000000000000"
        
        # Check collateral_seized structure
        assert result["collateral_seized"]["token"] == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
    ...
    def test_decode_liquidation_failure(
        ...
        with patch.object(processor.liquidation_event, 'process_log', side_effect=Exception("Decoding failed")):
            with pytest.raises(ValueError, match="Failed to decode Euler liquidation log"):
    ...
    def test_enrich_event_no_health_score(self, processor: EulerProtocolProcessor) -> None:
    ...
    def test_is_liquidation_transaction_euler_contract(
        ...
        with patch.object(processor.w3, 'keccak') as mock_keccak:
            mock_keccak.return_value.hex.return_value = "0x56864db9b7b0b85dfa9f4a8b8d5e1f9e4e6f6e9b1c7b8e9e6c5d4a3b2f1e0d9c8"
            
            result = processor.is_liquidation_transaction(sample_transaction, sample_logs)
    ...
    def test_constants(self, processor: EulerProtocolProcessor) -> None:
        ...
        assert processor.EULER_MAINNET_ADDRESS == "0x27182842E098f60e3D576794A5bFFb0777E025d3"
        assert processor.EXEC_PROXY_ADDRESS == "0x59828FdF7ee634AaaD3f58B19fDBa3b03E2a9d80"

    def test_liquidation_event_abi_structure(self, processor: EulerProtocolProcessor) -> None:
        ...
        expected_inputs = [
            "liquidator", "violator", "underlying", "collateral", 
            "repay", "yield", "healthScore", "baseDiscount", "discount"
        ]
        
        for expected_input in expected_inputs:
            assert expected_input in input_names

    def test_liquidation_bonus_calculation(self, processor: EulerProtocolProcessor) -> None:
    ...
    def test_liquidation_bonus_zero_discount(self, processor: EulerProtocolProcessor) -> None:
