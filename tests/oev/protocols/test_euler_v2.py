...
class TestEulerV2ProtocolProcessor:
    ...
    @pytest.fixture
    def sample_logs_single(
        self, sample_single_liquidation_log: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """Sample logs list with single liquidation for testing."""
        return [sample_single_liquidation_log]

    @pytest.fixture
    def sample_logs_batch(
        self, sample_batch_liquidation_log: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def test_decode_single_liquidation_success(
        ...
        with patch.object(
            ...
            "process_log",
            return_value=mock_single_liquidation_decoded_log,
        ) as mock_process_log:
            ...
            assert (
                result["collateral_seized"]["vault"]
                == "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
            )
            ...
            assert (
                result["liquidation_bonus"] == 1.05
            )  # 1.05 (discount value from mock)
            assert result["event_type"] == "single_liquidation"
            assert (
                result["transaction_hash"]
                == sample_single_liquidation_log["transactionHash"]
            )
            assert (
                result["block_number"] == sample_single_liquidation_log["blockNumber"]
            )
    ...
    def test_decode_batch_liquidation_success(
        ...
        with patch.object(
            processor.liquidation_event,
            "process_log",
            side_effect=Exception("Not single liquidation"),
        ):
            with patch.object(
                processor.batch_liquidation_event,
                "process_log",
                return_value=mock_batch_liquidation_decoded_log,
            ) as mock_batch_process_log:
                result = processor.decode_liquidation(sample_batch_liquidation_log)

                # Verify the batch method was called with the correct log
                mock_batch_process_log.assert_called_once_with(
                    sample_batch_liquidation_log
                )

                # Verify the returned structure for batch liquidation
                assert result["protocol"] == "euler_v2"
                assert (
                    result["liquidator"] == "0x742d35cc6634c0532925a3b8d8d5c0532925a3b8"
                )
                ...
                assert (
                    result["transaction_hash"]
                    == sample_batch_liquidation_log["transactionHash"]
                )
                assert (
                    result["block_number"]
                    == sample_batch_liquidation_log["blockNumber"]
                )
    ...
    def test_decode_liquidation_failure(
        ...
        with patch.object(
            processor.liquidation_event,
            "process_log",
            side_effect=Exception("Single liquidation failed"),
        ):
            with patch.object(
                processor.batch_liquidation_event,
                "process_log",
                side_effect=Exception("Batch liquidation failed"),
            ):
                with pytest.raises(
                    ValueError, match="Failed to decode Euler V2 liquidation log"
                ):
                    processor.decode_liquidation(sample_single_liquidation_log)

    def test_enrich_single_liquidation_event(
    ...
    def test_enrich_batch_liquidation_event(
        self, processor: EulerV2ProtocolProcessor
    ) -> None:
    ...
    def test_enrich_event_zero_amounts(
        self, processor: EulerV2ProtocolProcessor
    ) -> None:
    ...
    def test_constants(self, processor: EulerV2ProtocolProcessor) -> None:
        ...
        assert (
            processor.EULER_V2_FACTORY == "0x835482FE0532f169024d5E9410199369aAD5C77E"
        )
    ...
    def test_batch_liquidation_event_abi_structure(
        self, processor: EulerV2ProtocolProcessor
    ) -> None:
    ...
    def test_batch_liquidation_single_operation(
        self, processor: EulerV2ProtocolProcessor
    ) -> None:
    ...
    def test_batch_liquidation_multiple_operations(
        self, processor: EulerV2ProtocolProcessor
    ) -> None:
